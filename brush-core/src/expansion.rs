...
/// Expands `line` the way readline's `shell-expand-line` does.
///
/// Parameter expansion, command substitution and arithmetic expansion are applied with
/// quote removal. Literal source text, including whitespace and newlines, is preserved;
/// only unquoted expansion results undergo IFS splitting, with fields joined by spaces.
/// Brace and pathname expansion are left alone, as bash leaves them. A tilde prefix is
/// expanded only at the very start of the line: bash expands the whole line as one word,
/// so `~/x` at the start is a tilde prefix and `echo ~` or `a=~` is not.
///
/// # Arguments
///
/// * `shell` - The shell in which to expand.
/// * `params` - Execution parameters for any command substitutions.
/// * `line` - The line to expand.
pub async fn shell_expand_line<SE: extensions::ShellExtensions>(
    shell: &mut Shell<SE>,
    params: &ExecutionParameters,
    line: &str,
) -> Result<String, error::Error> {
    ...
    let mut expander = WordExpander::new_from_options(shell, params, &options);
    // The line is parsed as one word, so "at word start" is the start of the line. bash
    // does not treat the line as an assignment, so a tilde after a colon stays literal.
    expander.parser_options.tilde_expansion_after_colon = false;
    let pieces = brush_parser::word::parse(line, &expander.parser_options)?;
    let mut expansions = Vec::with_capacity(pieces.len());
    for piece in pieces {
        let Some(piece) = join_continued_lines(piece.piece) else {
            continue;
        };
        // Only top-level source text is protected. Text generated by a parameter
        // operator (such as ${v:-a:b}) must still undergo field splitting.
        let expansion = match piece {
            brush_parser::word::WordPiece::Text(text) => {
                Expansion::from(ExpansionPiece::Unsplittable(text))
            }
            other => expander.expand_word_piece(other).await?,
        };
...
///
/// A script's lexer joins continued lines before any word is parsed; an edit buffer has not
/// been through that, so they reach the word parser intact — as an escape sequence outside
/// quotes, and as literal text inside double quotes. Single-quoted and ANSI-C text keeps
/// them, as bash does. `None` means the piece was a continuation and nothing else.
fn join_continued_lines(
    piece: brush_parser::word::WordPiece,
) -> Option<brush_parser::word::WordPiece> {
    use brush_parser::word::WordPiece;

    fn join_all(
        pieces: Vec<brush_parser::word::WordPieceWithSource>,
    ) -> Vec<brush_parser::word::WordPieceWithSource> {
        pieces
            .into_iter()
            .filter_map(|piece| {
                Some(brush_parser::word::WordPieceWithSource {
                    piece: join_continued_lines(piece.piece)?,
                    ..piece
                })
            })
            .collect()
    ...
    match piece {
        WordPiece::EscapeSequence(s) if s == "\\\n" => None,
        WordPiece::Text(text) => Some(WordPiece::Text(text.replace("\\\n", ""))),
        WordPiece::DoubleQuotedSequence(pieces) => {
            Some(WordPiece::DoubleQuotedSequence(join_all(pieces)))
        }
        WordPiece::GettextDoubleQuotedSequence(pieces) => {
            Some(WordPiece::GettextDoubleQuotedSequence(join_all(pieces)))
        }
        other => Some(other),
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn shell_expand_line_matches_readline() -> Result<()> {
        let mut shell = crate::shell::Shell::builder().build().await?;
        let params = shell.default_exec_params();
        shell.set_env_global("V", ShellVariable::new("val"))?;

        // Quotes are removed and substitutions and arithmetic expanded; tilde and braces
        // are left as typed.
        let expanded = shell_expand_line(
            &mut shell,
            &params,
            r#"echo "$(echo A B)" ~ $((6*7)) {a,b} $V\ x 'q q'"#,
        )
        .await?;
        assert_eq!(expanded, "echo A B ~ 42 {a,b} val x q q");

        Ok(())
    }

    #[tokio::test]
    async fn shell_expand_line_expands_only_a_leading_tilde_prefix() -> Result<()> {
        // Checked against bash 5.3: the whole line is one word, so only a tilde prefix at
        // its start expands, and a bare `~` followed by a space is not one.
        let mut shell = Shell::builder().build().await?;
        let params = shell.default_exec_params();
        shell.set_env_global("V", ShellVariable::new("val"))?;
        shell.set_env_global("HOME", ShellVariable::new("/hh"))?;

        for (line, expected) in [
            ("~/x $V", "/hh/x val"),
            ("~ $V", "~ val"),
            ("~+ $V", "~+ val"),
            ("~nosuchuser/x $V", "~nosuchuser/x val"),
            (" ~/x $V", " ~/x val"),
            ("echo ~ $V", "echo ~ val"),
            ("echo a:~/x $V", "echo a:~/x val"),
            ("a=~/x $V", "a=~/x val"),
            ("\"~/x\" $V", "~/x val"),
        ] {
            assert_eq!(
    ...
    #[tokio::test]
    async fn shell_expand_line_preserves_literal_source() -> Result<()> {
        let mut shell = Shell::builder().build().await?;
        let params = shell.default_exec_params();
        shell.set_env_global("V", ShellVariable::new("x y"))?;
        shell.set_env_global("E", ShellVariable::new(""))?;

        for (line, expected) in [
            ("  echo   $V  ", "  echo   x y  "),
            ("echo\t$V\tend", "echo\tx y\tend"),
            ("echo\n$V\nbar", "echo\nx y\nbar"),
            ("echo \"$V\nbar\"", "echo x y\nbar"),
            ("echo $E end", "echo  end"),
            ("echo \"$E\" end", "echo  end"),
            ("echo $E", "echo "),
            ("$E", ""),
            ("echo a${V}b", "echo ax yb"),
            ("echo ${V}${V}", "echo x yx y"),
            ("echo \\\n$V", "echo x y"),
            ("echo \"a\\\n$V\"", "echo ax y"),
            ("echo 'a\\\nb'", "echo a\\\nb"),
            (" \t\n ", " \t\n "),
    ...
    #[tokio::test]
    async fn shell_expand_line_splits_only_expansion_results() -> Result<()> {
        let mut shell = Shell::builder().build().await?;
        let params = shell.default_exec_params();
        shell.set_env_global("IFS", ShellVariable::new(":"))?;
        shell.set_env_global("V", ShellVariable::new("x:y"))?;
        ...
        for (line, expected) in [
            ("echo a:b $V", "echo a:b x y"),
            (r"echo a\:b $V", "echo a:b x y"),
            (r#"echo "$V" '$V'"#, "echo x:y $V"),
            ("echo $V:$V", "echo x y:x y"),
            ("echo a${V}b", "echo ax yb"),
            ("echo ${V}${V}", "echo x yx y"),
            ("echo ${E:-a:b}", "echo a b"),
            (r#"echo ${E:-"a:b"}"#, "echo a:b"),
            ("echo $(printf 'x:y')", "echo x y"),
            ("echo \"$(printf 'x:y')\"", "echo x:y"),
            ("echo $((12))", "echo 12"),
            ("echo\n$V\nbar", "echo\nx y\nbar"),
        ] {
    ...
    #[tokio::test]
    async fn shell_expand_line_preserves_expansion_field_boundaries() -> Result<()> {
        let mut shell = Shell::builder().build().await?;
        let params = shell.default_exec_params();
        shell.set_env_global("E", ShellVariable::new(""))?;
    ...
    #[tokio::test]
    async fn shell_expand_line_preserves_empty_array_fields() -> Result<()> {
        let mut shell = Shell::builder().build().await?;
        let params = shell.default_exec_params();
        shell.set_env_global(
            "A",
        ...
        for (ifs, expected) in [
            ...
            shell.set_env_global("IFS", ShellVariable::new(ifs))?;
            assert_eq!(
                shell_expand_line(&mut shell, &params, "echo ${L[@]} end").await?,
                expected,
                "leading delimiter, IFS={ifs:?}"
            );
        }
        ...
        for (ifs, expected, quoted_star) in [(" \t\n", "echo aa bb end", "echo aa  bb end")] {
            ...
            assert_eq!(
                shell_expand_line(&mut shell, &params, "echo \"${A[@]}\" end").await?,
                "echo aa  bb end",
                "quoted array, IFS={ifs:?}"
            );
            assert_eq!(
                shell_expand_line(&mut shell, &params, "echo \"${A[*]}\" end").await?,
                quoted_star,
                "quoted concatenated array, IFS={ifs:?}"
            );
        }
