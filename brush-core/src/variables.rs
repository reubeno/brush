...
impl ShellVariable {
    ...
    /// Applies type-based transforms to a value string before storage.
    ///
    /// N.B. For `declare -i` (integer) variables, this performs simple `i64`
    /// parsing — NOT arithmetic expression evaluation. Callers that need full
    /// arithmetic evaluation (e.g., `x=20+5` → 25) must pre-evaluate the
    /// expression before calling `assign()`. The interpreter's `apply_assignment`
    /// in `interp.rs` handles this: it checks `is_treated_as_integer()` on the
    /// resolved target variable and evaluates the RHS through
    /// `arithmetic::expand_and_eval()` before assignment.
