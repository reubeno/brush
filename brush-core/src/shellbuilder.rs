//! Implementation of the builder pattern for the brush shell.
...
use crate::{
    DefaultShellBehavior, Shell, ShellBehavior, ShellFd, ShellRuntime as _, ShellVariable,
    builtins, error, openfiles, shell,
};
...
impl Shell {
    ...
    /// Create an instance of [Shell] using the builder syntax
    pub fn builder_with_behavior<SB: ShellBehavior>(
        behavior: SB,
    ) -> ShellBuilder<SB, shell_builder::Empty> {
...
pub struct CreateOptions<SB: ShellBehavior = DefaultShellBehavior> {
    ...
    pub key_bindings: Option<shell::KeyBindingsHelper>,
