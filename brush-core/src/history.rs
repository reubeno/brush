...
impl History {
    ...
    pub fn import(path: impl AsRef<Path>) -> Result<Self, error::Error> {
        ...
        for line in reader.lines() {
            ...
            history.items.push_back(item.id);
            history.id_map.insert(item.id, item);
        }
    ...
    /// # Arguments
    ///
    /// * `_id` - The unique identifier of the history item to update.
    /// * `_item` - The new history item to replace the old one.
    ...
    /// # Arguments
    ///
    /// * `_id` - The unique identifier of the history item to delete.
...
impl<'a> Search<'a> {
    ...
    fn increment_next_index(&mut self) {
