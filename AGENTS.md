...
- Unless explicitly requested, only major feature additions warrant an example in `brush-core/examples/`
...
- Use `anyhow` but typically only in tests
...
### Memory Management
- Shell instances can be cloned (creates new job manager, increments depth counter)
- Use `HashMap::clone()` judiciously - consider if reference borrowing is sufficient
...
- [ ] Verify examples work: `cargo run --package brush-core --example <name>`
