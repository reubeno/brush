...
impl RatatuiInputBackend {
    ...
    pub fn new() -> Result<Self, std::io::Error> {
        ...
        // PTY dimensions: The terminal pane (80% of screen) has a Block with borders.
        // The Block consumes 2 rows (top + bottom) and 2 cols (left + right).
        ...
        let pty_rows = terminal_pane_height.saturating_sub(1);
        let pty_cols = terminal_size.width.saturating_sub(2);
    ...
    pub fn draw_ui(&mut self) -> Result<(), std::io::Error> {
        ...
        self.terminal.draw(|f| {
            ...
            let (term_title, term_border_style) = if focused_pane == FocusedPane::Terminal {
                (
                    "Terminal [FOCUSED - Ctrl+Space to unfocus, Ctrl+Q to quit]",
                    Style::default()
                        .fg(Color::Green)
        ...
        if event::poll(Duration::from_millis(50))? {
            match event::read()? {
                Event::Key(key) if key.kind == KeyEventKind::Press => match key.code {
                    // Ctrl+Space switches focus between panes
                    ...
                    // Ctrl+Q quits the application unconditionally
                    ...
                    // Forward keyboard input to PTY when Terminal pane is focused
                    KeyCode::Char(c) if self.focused_pane == FocusedPane::Terminal => {
                    ...
                    KeyCode::Tab if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(vec![b'\t']));
                    }
                    KeyCode::Enter if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(vec![b'\r']));
                    }
                    KeyCode::Backspace if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(vec![0x7f]));
                    }
                    KeyCode::Esc if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(vec![0x1b]));
                    }
                    KeyCode::Delete if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(b"\x1b[3~".as_slice()));
                    }
                    KeyCode::Insert if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(b"\x1b[2~".as_slice()));
                    }
                    KeyCode::Home if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(b"\x1b[H".as_slice()));
                    }
                    KeyCode::End if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(b"\x1b[F".as_slice()));
                    }
                    KeyCode::PageUp if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(b"\x1b[5~".as_slice()));
                    }
                    KeyCode::PageDown if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(b"\x1b[6~".as_slice()));
                    }
                    KeyCode::Up if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(b"\x1b[A".as_slice()));
                    }
                    KeyCode::Down if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(b"\x1b[B".as_slice()));
                    }
                    KeyCode::Right if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(b"\x1b[C".as_slice()));
                    }
                    KeyCode::Left if self.focused_pane == FocusedPane::Terminal => {
                        let _ = self.pty_writer.try_send(Bytes::from(b"\x1b[D".as_slice()));
                    }
                    KeyCode::F(n) if self.focused_pane == FocusedPane::Terminal => {
