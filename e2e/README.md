...
| bash-tests | bash's own regression suite (`tests/run-*`), from a bash-5.3 build that also provides its helper binaries and the `--baseline` shell; one pytest case per `run-*` script, which is sourced verbatim with `diff` replaced by a function that captures the output for comparison against the `.right` file. `PLATFORM_DIFFS` in `tests/test_bash.py` excuses the few differences bash itself shows on Linux. Args go to pytest: `-- -k alias`, or `-- --subset minimal` for the scripts `run-minimal` runs |
