...
def wrap_numa(argv, core=None, numa_node=None):
    if numa_node is not None:
        argv = ["numactl", f"--cpunodebind={numa_node}", f"--membind={numa_node}"] + argv
    if core is not None:
        argv = ["taskset", "-c", str(core)] + argv
    return argv


def run(argv, core=None, numa_node=None):
    return subprocess.run(wrap_numa(argv, core, numa_node), capture_output=True, text=True)


def timed_run(argv, core=None, numa_node=None):
    start = time.perf_counter()
    result = run(argv, core, numa_node)
...
def peak_rss_kib(shell, script, args, core, numa_node=None):
...
def main():
    ...
    parser.add_argument("--numa-node", type=int, default=None, help="pin all runs to this NUMA node via numactl --cpunodebind/--membind")
    ...
    if not args.skip_parity:
        for wl in workloads:
            ...
            for shell in shells:
                ...
                outs = []
                for _ in batch:
                    _, out = timed_run(shell_argv(shell, wl["script"], wl["args"]), args.core, args.numa_node)
                    outs.append(out)
                outputs.add(outs[-1])
            if len(outputs) != 1:
        ...
        for wl in workloads:
            for shell in order:
                ...
                for _ in range(wl["batch"]):
                    elapsed, _ = timed_run(shell_argv(shell, wl["script"], wl["args"]), args.core, args.numa_node)
                    total += elapsed
    ...
    print(f"samples={args.samples}" + (f", pinned to cpu{args.core}" if args.core is not None else "") + (f", numa-node={args.numa_node}" if args.numa_node is not None else ""))
    print("=" * 76)
    ...
    for wl in workloads:
        ...
        row = f"   {wl['name']:>14}"
        for shell in shells:
            kib = peak_rss_kib(shell, wl["script"], wl["args"], args.core, args.numa_node)
